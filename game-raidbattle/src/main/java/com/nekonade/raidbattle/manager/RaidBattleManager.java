...
public class RaidBattleManager {
    ...
    public Constraint checkRaidBattleShouldBeFinished() {
        CopyOnWriteArrayList<RaidBattle.Enemy> enemies = raidBattle.getEnemies();
        if (enemies.size() == 0) {
            //不存在怪物
            return Constraint.EnemiesIsEmpty;
        }

        //超时
        if (isRaidBattleExpired()) {
            raidBattle.setFinish(true);
            raidBattle.setFailed(true);
            return Constraint.RaidBattleExpired;
        }
        //当所有玩家都撤退时,强制结束
        if (isRaidBattleAllPlayersRetired()) {
            raidBattle.setFinish(true);
            raidBattle.setFailed(true);
            return Constraint.AllPlayerRetried;
        }
        ...
        if (enemies.stream().noneMatch(enemy -> enemy.getKey() == 1)) {
            ...
            if (enemies.stream().noneMatch(this::isEnemyAlive)) {
                //没有存活
                raidBattle.setFinish(true);
                return Constraint.RaidBattleFinish;
            }
        } else {
            //当存在Key Monster时
        ...
        //尚有存活
        return Constraint.RaidBattleDoNothing;
    }
