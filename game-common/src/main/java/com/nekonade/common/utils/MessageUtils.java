...
public class MessageUtils {
    ...
    public static <T extends GameMessagePackage>void CalcMessageDealTime(Logger logger, T gameMessage){
        ...
        if(time == 0 || time >= 1600000000000L){
            ...
            logger.warn("MessageId:{} TotalTime:{} Player:{} RaidId:{} 处理有较大延迟(>={})",header.getMessageId(),time,header.getPlayerId(),attribute.getRaidId(),longTime);
        }else{
            logger.info("MessageId:{} TotalTime:{} Player:{} RaidId:{}",header.getMessageId(),time,header.getPlayerId(),attribute.getRaidId());
        }
    }
