...
public abstract class AbstractRaidBattleChannelHandlerContext {
    ...
    static void invokeChannelRegistered(final AbstractRaidBattleChannelHandlerContext next, String symbolId, RaidBattleChannelPromise promise) {
        EventExecutor executor = next.executor();
        if (executor.inEventLoop()) {
            next.invokeChannelRegistered(symbolId, promise);
        } else {
            executor.execute(new Runnable() {
                @Override
                public void run() {
    ...
    public AbstractRaidBattleChannelHandlerContext fireChannelRegistered(String symbolId, RaidBattleChannelPromise promise) {
        invokeChannelRegistered(findContextInbound(), symbolId, promise);
        return this;
    }

    private void invokeChannelRegistered(String symbolId, RaidBattleChannelPromise promise) {

        try {
            ((RaidBattleChannelInboundHandler) handler()).channelRegister(this, symbolId, promise);
        } catch (Throwable t) {
            notifyHandlerException(t);
        }
    }
